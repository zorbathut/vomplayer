...
{
    // Click hit-test half-width in pixels: a click within this many px of a marker's X snaps to it. The full click target is 2x this wide. Adjacent targets don't overlap — ChapterHitTest returns the *nearest* marker, so two bands that would overlap instead split at the midpoint between the two markers (see ChapterHitTest's class comment).
    private const double ClickToleranceTpx = 12.0;
